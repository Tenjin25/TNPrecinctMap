...
/**
 * Apply block-chain / official-VTD20 crosswalks onto statewide contest JSONs.
 *
 * Uses:
 *   Data/tn_vtd_2020.geojson  (official Census VTD20 map layer)
 *   Data/crosswalks/tn_precinct_to_vtd20_blockchain_{year}.csv
 *
 * Does not touch Data/district_contests*. Build crosswalks first with:
...
const CANONICAL_CSV_DIR = path.join(DATA_DIR, "canonical_precinct_csvs");
const PRECINCT_GEOJSON = path.join(DATA_DIR, "tn_vtd_2020.geojson");
const CROSSWALK_PREFIX = "tn_precinct_to_vtd20_blockchain_";

function sourceCsv(fileName) {
  const canonicalPath = path.join(CANONICAL_CSV_DIR, fileName);
...
const TARGETS = [
  {
    ...
    sourceCsv: sourceCsv("20001107__tn__general__president__precinct.csv"),
  ...
  },
  {
    ...
    sourceCsv: sourceCsv("20001107__tn__general__senate__precinct.csv"),
  ...
  },
  {
    ...
    sourceCsv: sourceCsv("20021105__tn__general__governor__precinct.csv"),
  ...
  },
  {
    year: 2002,
    contestType: "us_senate",
    sourceCsv: sourceCsv("20021105__tn__general__senate__precinct.csv"),
  ...
  },
  {
    year: 2004,
    contestType: "president",
    sourceCsv: sourceCsv("20041102__tn__general__precinct.csv"),
  ...
  },
  {
    year: 2006,
    contestType: "governor",
    sourceCsv: sourceCsv("20061107__tn__general__governor__precinct.csv"),
  ...
  },
  {
    year: 2006,
    contestType: "us_senate",
    sourceCsv: sourceCsv("20061107__tn__general__senate__precinct.csv"),
  ...
  },
  {
    year: 2008,
    contestType: "president",
    sourceCsv: sourceCsv("20081104__tn__general__precinct.csv"),
  ...
  },
  {
    year: 2010,
    contestType: "governor",
    sourceCsv: sourceCsv("20101102__tn__general__precinct.csv"),
  ...
  },
  {
    year: 2012,
    contestType: "president",
    sourceCsv: sourceCsv("20121106__tn__general__precinct.csv"),
  ...
  },
  {
    year: 2014,
    contestType: "governor",
    sourceCsv: sourceCsv("20141104__tn__general__precinct.csv"),
  ...
  },
  {
    year: 2016,
    contestType: "president",
    sourceCsv: sourceCsv("20161108__tn__general__precinct.csv"),
  ...
  },
  {
    year: 2018,
    contestType: "governor",
    sourceCsv: sourceCsv("20181106__tn__general__precinct.csv"),
  ...
  },
  {
    year: 2020,
    contestType: "president",
    sourceCsv: sourceCsv("20201103__tn__general__precinct.csv"),
  ...
  },
  {
    year: 2022,
    contestType: "governor",
    sourceCsv: sourceCsv("20221108__tn__general__governor__precinct.csv"),
  ...
  },
  {
    year: 2024,
    contestType: "president",
    sourceCsv: sourceCsv("20241105__tn__general__precinct.csv"),
...
function excelSerialAliases(precinct) {
  // Decode Excel date-serial artifacts like 44562 -> 1 1 / 01 01 (2022-01-01).
  // Only month-day in decoded order — day-month swaps collide (1-2 vs 2-1).
  const s = normPrecinct(precinct);
  if (!/^\d{5}$/.test(s)) return [];
  const serial = Number(s);
  if (serial < 43000 || serial > 50000) return [];
  ...
  const out = [
    normPrecinct(`${mm}-${dd}`),
    normPrecinct(`${String(mm).padStart(2, "0")}-${String(dd).padStart(2, "0")}`),
  ];
  return [...new Set(out.filter(Boolean))];
}

function precinctLookupKeys(county, precinct) {
  const base = normPrecinct(precinct);
  const keys = [`${county}\t${base}`];
  for (const alias of excelSerialAliases(base)) {
    keys.push(`${county}\t${alias}`);
  }
  return keys;
}
...
function loadCountyNameByFips() {
  const countyPath = path.join(DATA_DIR, "tl_2020_47_county20.geojson");
  if (!fs.existsSync(countyPath)) return new Map();
  const geo = JSON.parse(fs.readFileSync(countyPath, "utf8"));
  const out = new Map();
  for (const feature of geo.features || []) {
    const props = feature.properties || {};
    const fips = String(props.COUNTYFP20 || props.COUNTYFP || "").replace(/[^0-9]/g, "").slice(-3).padStart(3, "0");
    const name = normCounty(props.NAME20 || props.NAME || "");
    if (fips && name) out.set(fips, name);
  }
  return out;
}

function loadOverlayLabels() {
  if (!fs.existsSync(PRECINCT_GEOJSON)) {
    throw new Error(`Missing Census VTD20 GeoJSON: ${PRECINCT_GEOJSON}`);
  }
  const geo = JSON.parse(fs.readFileSync(PRECINCT_GEOJSON, "utf8"));
  const countyByFips = loadCountyNameByFips();
  const labels = new Map();
  const allLabels = new Set();
  for (const feature of geo.features || []) {
    const props = feature.properties || {};
    const fips = String(props.COUNTYFP20 || props.COUNTYFP || "").replace(/[^0-9]/g, "").slice(-3).padStart(3, "0");
    const county = normCounty(
      props.county_norm || props.county_nam || countyByFips.get(fips) || ""
    );
    let precId = normSpace(props.prec_id || props.VTDST20 || props.vtdst20 || "");
    if (/^\d+$/.test(precId)) precId = precId.padStart(6, "0");
  ...
  if (!allLabels.size) {
    throw new Error(`No drawable Census VTD20 labels found in ${PRECINCT_GEOJSON}`);
  }
  return { byCounty: labels, allLabels };
}

function loadWeightedCrosswalk(year) {
  const file = path.join(CROSSWALKS_DIR, `${CROSSWALK_PREFIX}${year}.csv`);
  if (!fs.existsSync(file)) {
    throw new Error(`Missing block-chain crosswalk for ${year}: ${file}. Run build_tn_block_chain_frontend_crosswalks.py first.`);
  }
  ...
  for (const row of rows) {
    ...
    let dst = normSpace(row.dst_vtd20);
    if (/^\d+$/.test(dst)) dst = dst.padStart(6, "0");
    const weight = Number(row.weight);
    if (!county || !precinct || !dst || !Number.isFinite(weight) || weight <= 0) continue;
    // Index under the raw key and Excel-serial decodes so governor-style
    // labels like "1-1" join crosswalk rows stored as "44562".
    for (const key of precinctLookupKeys(county, precinct)) {
...
function addAllocatedVotes(totalsByLabel, crosswalk, overlayLabelSet, county, precinct, party, candidate, votes, stats) {
  ...
  let allocs = null;
  for (const key of precinctLookupKeys(county, precinct)) {
    allocs = crosswalk.get(key);
    if (allocs && allocs.length) break;
  }
...
function patchContest(target, overlayLabels, options = {}) {
  ...
  for (const [county, items] of computedByCounty.entries()) {
    ...
    if (!force && (coverage < 0.60 || voteCoverage < 0.40 || !existingTotalVotes || !computedTotalVotes)) {
      continue;
    }
    if (force && !forceAll && (!computedTotalVotes || coverage < 0.25)) {
      continue;
    }
    if (forceAll && !computedTotalVotes) {
      continue;
    }
    ...
    if (!force && newZeroCount >= oldZeroCount) {
      continue;
    }
  ...
  const out = {
    ...
    meta: {
      ...(existing.meta || {}),
      source: existing.meta?.source || "tn_precinct_csv_block_chain_to_dra_v07",
      rows: rows.length,
      synthetic_zero_rows_added: zeroFill,
      weighted_vtd20_allocation: true,
      block_chain_frontend_allocation: true,
      weighted_vtd20_source: path.relative(ROOT, crosswalkFile).replace(/\\/g, "/"),
      frontend_precinct_geojson: path.relative(ROOT, PRECINCT_GEOJSON).replace(/\\/g, "/"),
