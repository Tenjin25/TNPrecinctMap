...
- Added `Scripts/normalize_tn_precinct_csvs.js` and `Data/canonical_precinct_csvs/` so precinct rebuilds can use a consistent `county,precinct,office,district,party,candidate,votes` input shape while raw source exports, including 2024 `PRCTSEQ` fields, remain available for crosswalk tooling.
...
- Added `Scripts/fill_tn_vtd20_gap_rows_from_prior.js` as a second-stage statewide precinct coverage pass:
